...
(async () => {
  ...
  app.get('/cars', (req: Request, res: Response) => {
    ...
    if (make) {
      const selection = cars.filter((car) => car.make === make);
  ...
  app.post('/cars', (req: Request, res: Response) => {
    ...
    if (!make || !type || !model || !cost || !id) {
      return res.status(400).send('Your car details are issing');
    }

    cars.push({ id, make, model, type, cost });
    return res.status(201).send('Your entry was successful');
  });
